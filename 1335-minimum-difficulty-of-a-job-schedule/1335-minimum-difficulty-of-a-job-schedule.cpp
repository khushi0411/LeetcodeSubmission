...
public:
     int dp[12][305];  
    //dp[d][i] is the answer that we have d days left, and we have finished all the tasks before arr[i](exclusive)
    vector<int> arr; 
    
    int maxquery(int i,int j) {  
        //return the maximum value of arr[i,j]
        int maxi = -1;
    ...
    int minDifficulty(vector<int>& arr, int day) {
        ...
        for (int d = 2; d <= day; d++) {
            for (int i = 0; i < n; i++) {
                if (n-i < d) {
        //too many days left, not possible to use them all
                    dp[d][i] = 1e6;
                    continue;
                }
                for (int j = i; j < n; j++) {
