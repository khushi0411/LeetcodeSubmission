...
public:
    int sumSubarrayMins(vector<int>& arr) {  
      ...
      long sum = 0;
      vector<int> dp(n); 
        // dp[i] stores the sum of subarrays ending at i
      
      for (int i = 0; i < n; i++) {
        
        while (!s.empty() && arr[s.top()] >= arr[i])
            // find the leftmost index whose value is smaller than arr[i].
          s.pop();
        
        if (s.size()) {
            // there is a smaller value than arr[i] to the left
          ...
          dp[i] = dp[prev] + (i - prev)*arr[i];
            ...
            // No smaller value in arr - all subarrays ending at i have minimum value arr[i].
          dp[i] = (i+1)*arr[i];
        ...
        s.push(i);
          // add the index to the stack as all larger values have been removed
      }
